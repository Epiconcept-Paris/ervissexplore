# Fetching ERVISS Data

## Introduction

The `ervissexplore` package provides easy access to all data published
in the [EU-ECDC Respiratory Viruses Weekly
...
|--------------------------|----------------------------------------------------------------------------------|-----------------------------------------|-------------------------------------------------------|
| `"positivity"`           | [`get_sentineltests_positivity()`](../reference/get_sentineltests_positivity.md) | `sentinelTestsDetectionsPositivity.csv` | Sentinel test positivity rates by pathogen            |
| `"variants"`             | [`get_erviss_variants()`](../reference/get_erviss_variants.md)                   | `variants.csv`                          | SARS-CoV-2 variant proportions                        |
| `"ili_ari_rates"`        | [`get_ili_ari_rates()`](../reference/get_ili_ari_rates.md)                       | `ILIARIRates.csv`                       | ILI/ARI consultation rates by age group               |
| `"sari_rates"`           | [`get_sari_rates()`](../reference/get_sari_rates.md)                             | `SARIRates.csv`                         | SARI rates by age group                               |
| `"sari_positivity"`      | [`get_sari_positivity()`](../reference/get_sari_positivity.md)                   | `SARITestsDetectionsPositivity.csv`     | SARI virological data (tests, detections, positivity) |
| `"nonsentinel_severity"` | [`get_nonsentinel_severity()`](../reference/get_nonsentinel_severity.md)         | `nonSentinelSeverity.csv`               | Non-sentinel severity (deaths, hospitalisations, ICU) |
| `"nonsentinel_tests"`    | [`get_nonsentinel_tests()`](../reference/get_nonsentinel_tests.md)               | `nonSentinelTestsDetections.csv`        | Non-sentinel tests and detections                     |

Each source has a dedicated `get_*()` function, or you can use the
generic `get_erviss_data(type = ...)` function.
...
Tests, detections, and positivity rates from SARI virological
surveillance.
...
Hospital admissions, ICU admissions, ICU inpatients, hospital
inpatients, and deaths from non-sentinel sources.
...
[`get_erviss_data()`](../reference/get_erviss_data.md) with the `type`
parameter:
...
analyses**. The ERVISS repository stores weekly snapshots of the data,
so you can retrieve the exact data that was available at a given date.
...
To see available snapshot dates, visit the [EU-ECDC snapshots
directory](https://github.com/EU-ECDC/Respiratory_viruses_weekly_data/tree/main/data/snapshots).
...
[`get_sentineltests_positivity_url()`](../reference/get_sentineltests_positivity_url.md),
[`get_ili_ari_rates_url()`](../reference/get_ili_ari_rates_url.md),
etc.).
...
All functions return `data.table` objects. You can use `data.table`
syntax or convert to a `data.frame` / `tibble` for your preferred
workflow:
